...
def calculates_results_stats(results_dic):
    ...
    """   
#     Creating an empty dict to hold the statistics
    results_stats_dic= dict()
    
#     initiliazing all counts to zero
...
#     calculating the stats by looping through the dic.

    for key in results_dic:
        
#       5a.  if pet label= classifier label then there's a match and count +1
       if results_dic[key][2] == 1:
          results_stats_dic['n_match'] += 1
#             if pet label= classifier label AND pet label IS A DOG( dog breed is correct) 
       if (results_dic[key][2]== 1 and results_dic[key][3]== 1):
          results_stats_dic['n_correct_breed'] +=1
#      counting number of dog images
       if results_dic[key][3]==1:
          results_stats_dic['n_dogs_img'] +=1
#             no of correct dog classifications
          if results_dic[key][4]== 1:
             results_stats_dic['n_correct_dogs'] += 1
#        else if both labels = IS NOT A DOG(no. of correct 'not dog' classification)
       else:     
    ...
    if results_stats_dic['n_correct_notdogs'] != None:
